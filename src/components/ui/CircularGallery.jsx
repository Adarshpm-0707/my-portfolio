...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // object-fit: cover — scale image to fill plane, crop overflow
          float planeAspect = uPlaneSizes.x / uPlaneSizes.y;
          float imageAspect = uImageSizes.x / uImageSizes.y;
          vec2 uv;
          if (imageAspect > planeAspect) {
            // image is wider than plane — fit height, crop sides
            float scale = planeAspect / imageAspect;
            uv = vec2(vUv.x * scale + (1.0 - scale) * 0.5, vUv.y);
          } else {
            // image is taller than plane — fit width, crop top/bottom
            float scale = imageAspect / planeAspect;
            uv = vec2(vUv.x, vUv.y * scale + (1.0 - scale) * 0.5);
          }
  ...
  onResize({ screen, viewport } = {}) {
    ...
    if (isMobile) {
      // Mobile: each plane fills ~85% of viewport width, 3:4 height ratio
      this.plane.scale.x = this.viewport.width * 0.85;
      this.plane.scale.y = this.plane.scale.x * (3 / 4);
    } else {
      // Desktop/tablet: plane height = 78% of viewport, 16:9 width
      this.plane.scale.y = this.viewport.height * 0.78;
      this.plane.scale.x = this.plane.scale.y * (16 / 9);
    }
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    this.padding = isMobile ? 0.5 : 1.5;
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600`, text: 'Project One' },
      { image: `https://picsum.photos/seed/2/800/600`, text: 'Project Two' },
      { image: `https://picsum.photos/seed/3/800/600`, text: 'Project Three' },
    ];
