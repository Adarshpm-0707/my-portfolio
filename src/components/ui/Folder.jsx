...
const Folder = ({ 
  ...
  const getOpenTransform = index => {
    if (isMobile) {
      if (index === 0) return 'translate(-92%, -55%) rotate(-8deg)';
      if (index === 1) return 'translate(5%, -55%) rotate(8deg)';
      if (index === 2) return 'translate(-50%, -80%) rotate(3deg)';
    } else {
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              transform: (open || isHovered) ? 'skew(15deg) scaleY(0.6)' : 'skew(0deg) scaleY(1)'
            }}
          ></div>
          <div
            className={`absolute z-30 w-full h-full origin-bottom transition-all duration-300 ease-in-out`}
